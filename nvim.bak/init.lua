...
-- See `:help mapleader`
--  NOTE: Must happen before plugins are loaded (otherwise wrong leader
--  will be used)
vim.g.mapleader = " "
...
--  startup-time.
--  Remove this option if you want your OS clipboard to remain
--  independent.
--  See `:help 'clipboard'`
...
--    :Lazy
--
--  You can press `?` in this menu for help. Use `:q` to close the
--  window
--
...
require("lazy").setup({
	-- NOTE: Plugins can be added with a link (or for a github repo:
  -- 'owner/repo' link).
  ...
  -- `setup()` function, forcing the plugin to be loaded.
	...
	-- Alternatively, use `config = function() ... end` for full control
  -- over the configuration.
	...
	-- NOTE: Plugins can also be configured to run Lua code when they are
  -- loaded.
	--
	-- This is often very useful to both group configuration, as well as
  -- handle lazy loading plugins that don't need to be loaded
  -- immediately at startup.
	--
	-- For example, in the following configuration, we use:
	--  event = 'VimEnter'
	--
	-- which loads which-key before all the UI elements are loaded. Events 
  -- can be normal autocommands events (`:help autocmd-events`).
	--
	-- Then, because we use the `opts` key (recommended), the 
  -- configuration runs after the plugin has been loaded as 
  -- `require(MODULE).setup(opts)`.
	...
	{ -- Useful plugin to show you pending keybinds.
      ...
      -- (milliseconds)
        ...
        -- table which will use the
				-- default which-key.nvim defined Nerd Font icons, otherwise
	...
	--
	-- The dependencies are proper plugin specifications as well -
  -- anything you do for a plugin at the top level, you can do for a
  -- dependency.
	--
	-- Use the `dependencies` key to specify the dependencies of a
  -- particular plugin
	...
	{ -- Fuzzy Finder (files, lsp, etc)
      ...
      -- finder", it can search
			-- many different aspects of Neovim, your workspace, LSP, and
      -- more!
			--
      ...
      -- something like:
			--  :Telescope help_tags
			--
			-- After running this command, a window will open up and you're
      -- able to type in the prompt window. You'll see a list of
      -- `help_tags` options and
	...
	},
	{
      ...
      -- Neovim
			-- Mason must be loaded before its dependents so we need to set
      -- it up here.
      ...
      -- These Language Servers (sometimes called LSP servers, but
      -- that's kind of like ATM Machine) are standalone processes 
      ...
      -- installed separately from
			-- Neovim. This is where `mason` and related plugins come into 
      -- play.
			--
			-- If you're wondering about lsp vs treesitter, you can check 
      -- out the wonderfully
			-- and elegantly composed help section, `:help lsp-vs-treesitter`

			--  This function gets run when an LSP attaches to a particular 
      --  buffer.
			--    That is to say, every time a new file is opened that is 
      --    associated with
      ...
      --    `rust_analyzer`) this
          ...
          -- and as such it is possible
					-- to define small helper and utility functions so you don't 
          -- have to repeat yourself.
					--
					-- In this case, we create a function that lets us more 
          -- easily define mappings specific
					-- for LSP related items. It sets the mode, buffer and 
          ...
          --  function is defined, etc.
              ...
              "[G]oto [D]efinition")
					...
					-- WARN: This is not Goto Definition, this is Goto
          -- Declaration.
