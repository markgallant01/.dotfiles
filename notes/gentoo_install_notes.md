...
The easiest thing to do is boot into a live Linux iso of some kind with
a graphical desktop all set up. This makes it nice and easy to connect
to the internet and do everything you'll have to do. Linux Mint is a
good choice.

Download a fresh iso and use the dd utility to make a bootable live usb:

`$ dd bs=4M if=path/to/archlinux.iso of=/dev/flashDrive status=progress; sync`

Once you're booted into a live environment, get set up and ready to
install.
Make sure you're connected to the internet, pull up a terminal and a
web browser and get comfy
I usually disable pointer acceleration too and you might need to install
vim depending on the live iso you're using:

(for Mint and other Debian derivatives):

`$ sudo apt install neovim`

Once you're ready to go, open the [Gentoo Installation Alternatives](https://wiki.gentoo.org/wiki/Installation_alternatives#Installation_from_non-Gentoo_live_environments) page.

That page has special instructions that need to be followed at various
points in the install process, so keep it up for reference.

In the terminal, switch to the root user so you don't need to type
sudo before every command:

`$ sudo su`

Then, create the /mnt/gentoo mount point:

`# mkdir /mnt/gentoo`

Begin following the handbook from the [Disks](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Disks) page.


# 2. Preparing the disks

For the partition setup, use 3 partitions as follows:
    
| partition | filesystem | size      | description          |
| --------- | ---------- | --------- | -------------------- |
| /dev/sdx1 | fat32      | 1 GiB     | EFI (boot) partition |
| /dev/sdx2 | swap       | RAM * 1.5 | Swap partition       |
| /dev/sdx3 | xfs        | remainder | root partition       |

For explanations of the setup or alternatives, consult the handbook page. Use cfdisk
to make the partitions, it's easier than fdisk. If cfdisk isn't available for some
reason, fdisk instructions are given in the handbook. After making the partitions,
create the filesystems.

EFI partition gets formatted as fat32:

`# mkfs.vfat -F 32 /dev/sdx1`

Root partition as xfs:

`# mkfs.xfs -c options=/usr/share/xfsprogs/mkfs/lts_6.6.conf /dev/sdx3`
...
Set the current directory to the location of the mount used for the install:

`# cd /mnt/gentoo`

Download the stage tarball. In a browser, open the [downloads page](https://www.gentoo.org/downloads/#other-arches).

Select your architecture, probably amd64, and choose a stage file, probably 'desktop
profile | systemd'

Copy the URL for the stage file download and paste it into a wget command:

`# wget <STAGE_FILE_URL>`

The URL as of our last installation is [here](https://distfiles.gentoo.org/releases/amd64/autobuilds/20260316T093103Z/stage3-amd64-desktop-systemd-20260316T093103Z.tar.xz) for
convenience.

Extract the tar file with the following command:

Note that this command is slightly different from the one in the handbook, it's
to be used when installing from a live Linux environment:

`# tar --numeric-owner --xattrs -xvJpf stage3-*.tar.xz -C /mnt/gentoo`

Use an editor to set the initial make.conf variables:
...
Note that These are flags to ADD to the variables. if the variables already have
some default options, add these in addition to those already there. These are just
the initial basic settings.

The values for -jN and -lN should be something that makes sense for the target system.
Reference [this page](https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Stage#Configuring_compile_options) for more info.


# 4. Installing base system

Copy DNS info so the chrooted environment can still connect to the internet:
...
List any news items with portage:
...
Edit the make.conf file and set these variables. You'll need to use nano just
this once before vim is installed. These are flags to ADD to the variables.
If the variables already have some default options, add these in addition to
those already there.
...
EMERGE_DEFAULT_OPTS="--jobs=X --load-average=X --usepkg=y"
ACCEPT_LICENSE="*"
```

Set the `PORTAGE_BINHOST` variable to these URLs for downloading binary packages:
...
> Note: Maybe update this section to use the binrepos.conf file because
the way we do it here is technically deprecated

Run this command so that binary packages can be used:

`# getuto`

Now update the mirrors with the mirrorlist tool. first install it:

`# emerge -a --oneshot app-portage/mirrorselect`

Run this command to select the 5 fastest mirrors:

`# mirrorselect -s5 -o >> /etc/portage/make.conf`

Install neovim so it's easier to edit these files:

`# emerge -a app-editors/neovim`

Select the correct profile (should match the stage file used earlier):
...
```
# emerge -a --oneshot app-portage/cpuid2cpuflags
# echo "*/* $(cpuid2cpuflags)" > /etc/portage/package.use/00cpu-flags
```

Unset the default GPU flag and set an appropriate one.
`GPU_DRIVER_NAME` will likely be one of `nvidia`, `intel`, or 
`amdgpu radeonsi`. consult the handbook for a comprehensive list.

```
"/etc/portage/package.use/00video_cards"
----------------------------------------
*/* VIDEO_CARDS: -* <GPU_DRIVER_NAME>
```

Update the system to account for any changes made so far:

`# emerge -a --update --deep --changed-use @world`

Remove and unused packages after updating:
...
Set locales by un-commenting the relevant line in the locale.gen file:
...
List the locales and then select the correct one:
...
# emerge -a sys-firmware/sof-firmware
```

Install CPU microcode updates. AMD CPUs are included in the above firmware
package. Intel CPUs require:

`# emerge -a sys-firmware/intel-microcode`

To choose the bootloader, create this file with the following flags:
...
And symlink it with this other file:
...
Then install the installkernel:

`# emerge -a sys-kernel/installkernel`

Set this flag so the installkernel handles the initramfs with dracut:
...
Some bootloaders need the root partition explicitly defined:

```
# mkdir /etc/dracut.conf.d
# blkid >> /etc/dracut.conf.d/00-installkernel.conf
```

Go into that new file and identify the root partition and set it up like
so (note leading and trailing spaces before and after the UUID):

```
"/etc/dracut.conf.d/00-installkernel.conf"
kernel_cmdline=" root=UUID=XXXXXXXXX... "
```

> Note: Do we need UKI? no idea
...
Add this USE flag globally to make.conf so that the initramfs are
automatically rebuilt when the kernel updgrades:
...
Set up your fstab file. The handbook does this manually but genfstab is easy.

`# emerge -a sys-fs/genfstab`

Generate the fstab using UUID labels
...
Set up network manager by first adding the USE flag to make.conf:
...
> Note: I'm not sure how necessary this is so come back to this. Consult the
handbook for now
...
Run some finalization commands for systemd:
...
> Note: The second command opened a TUI and asked me about the keyboard layout
and nothing else. I think it might only prompt for stuff that isn't explicitly set.

`# systemctl preset-all --preset-mode=enable-only`

# 7. Installing system tools

Install a file indexer to speed up file location:
...
Timesync service for the clock:

`# systemctl enable systemd-timesyncd.service`

Filesystem tools:

`# emerge -a sys-fs/xfsprogs sys-fs/dosfstools`

Recommended for correct scheduling behavior:

`# emerge -a sys-block/io-scheduler-udev-rules`

Portage tools:

`# emerge -a app-portage/gentoolkit`

> Note: Automate this whole section. Also add the eix program.

# 8. Configuring the bootloader

Systemd-boot should already be installed

`# bootctl install`

Check for a new boot entry with this:

`# bootctl list`

Exit the chrooted environment and unmount all partitions:
...
Finally, reboot. Be sure to remove the installation medium before restarting.

# Post-install

> Note: Need to install app-admin/sudo at some point before exiting the chroot

> Note: This whole section needs to be reworked for Gentoo

This section sets up the newly installed system. Login to the new system as root.

Setup a non-root user and set the password. User is in the wheel group so they can sudo
and the pipewire group for audio scheduling stuff.

```
# useradd -m -G wheel,pipewire <username>
# passwd *user*
```
...
If nvidia drivers were installed, you must edit the file /etc/mkinitcpio.conf to
remove the 'kms' module from the HOOKS array. This ensures the nouveau module will
not be loaded during boot:
...
Gentoo installed stuff at this point:
...
`$ sudo emerge -a gui-wm/niri`

Niri has a bunch of other stuff that should be installed. Consult [this page](https://niri-wm.github.io/niri/Important-Software.html).

Installed noctalia-shell from guru following the official instructions. add
that stuff here. check optional dependencies

Optional stuff i'm installing:
...
There's more optional deps I think. check [here](https://docs.noctalia.dev/getting-started/installation/#manual-install) at some point.

> STUFF TO STILL DO:

> figure out what else is needed for noctalia shell. there's more optional
stuff that's missing

> look at the settings and junk and change what you want

> rip settings and keybinds and stuff from DMS

> add the settings to be backed up in our config file. figure out a way to get
our /etc/portage stuff in there too.

> maybe number the Niri workstations so they stay around when empty

> package install script

> customize foot terminal

> should we add the vaapi flag to the USE flags? consult gentoo VAAPI page

Add a GPU section or maybe add this stuff above when setting the VIDEO_CARDS
variable because we need some extra stuff for intel at least:

`$ sudo emerge -a media-libs/libva-intel-media-driver`

That enables hardware accel for intel iGPU's. Do we need this for other GPU's too?

After that we've installed discord and fastfetch. noctalia-shell needs fastfetch for sys info

`$ sudo emerge -a app-misc/fastfetch app-im/discord`

Configure foot. we created the config file and changed some stuff. add it to the
config and link it up.

Set up audio by doing this:
...
Pipewire should be enabled by default on the desktop profile so this should be
all you need to do.

-searching with portage is slow so read about eix and add a section for it

-add a section to start / enable bluetooth (it should all be installed automatically)

-installed eix because it's a lot faster for querying package stuff:

	$ sudo emerge -a app-portage/eix

-discord wouldn't run without xwayland-satellite so include that with niri:
	
	$ sudo emerge -a gui-apps/xwayland-satellite

	-there's stuff you have to do. unmask the testing thing.

-add a section for development stuff. so far we've installed:

	$ sudo emerge -a net-libs/nodejs

-add Steam install instructions. Use the wiki page and follow the flatpak instructions
 so we don't need to compile a ton of 32 bit libraries

-set up fonts somewhere in here. There's a wiki page for it. I hate having those missing
 glyph symbols so make sure we have everything we need

-installed pandoc:

	$ sudo emerge -a app-text/pandoc-bin

-installed Thunar file manager

    $ sudo emerge -a xfce-base/thunar

    -to make this work as the default file picker you have to add a line to the portal
     config:
...
To-do:
    -wl-clipboard enables copy and paste from clipboard to neovim so install
     that
